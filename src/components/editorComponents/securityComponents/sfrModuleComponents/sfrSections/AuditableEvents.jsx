 ...
 * @param sfrType the sfr type used for the audit table (mandatory, optional, objective, selectionBased, implementationDependent, additional)
 * @param uuid the uuid of the sfr base pp (used only for additional sfrs)
 * @param toeAudit the specific toe audit by type (not used for additional sfrs)
 * @param dialogTitle the dialog title used for the toggle confirmation message
 * @constructor
 */
function AuditableEvents({ sfrType, uuid, toeAudit, dialogTitle }) {
  // Prop Validation
  AuditableEvents.propTypes = {
    sfrType: PropTypes.string.isRequired,
    uuid: PropTypes.string,
    toeAudit: PropTypes.object,
  ...
  let audit = sfrType === "additional" ? additionalSfrs?.audit || {} : toeAudit || {};
  let { section: defaultSection, auditTable: defaultAuditTable } = defaultAudit;
  const { isAudit = true, section = deepCopy(defaultSection), auditTable = deepCopy(defaultAuditTable), eventsTableOpen = false, open = true } = audit;
  ...
  /**
   * Updates the expand/collapse value of the audit events table
   */
  const handleAuditEventsTableCollapse = () => {
    // Update the expand/collapse value
    updateAudit("eventsTableOpen", !eventsTableOpen);
  };
  /**
   *
  ...
  const updateAudit = (key, value) => {
    ...
    if (sfrType === "additional") {
      updateAdditionalSfr(uuid, "audit", auditCopy);
    } else {
      updateToeSfr(sfrType, "audit", auditCopy);
    }
  };

  // Components
  /**
   * Gets the auditable events toggle section
   * @returns {JSX.Element}
   */
   ...
   * @constructor
   */
  const AuditSection = (type) => {
    const tooltip = "";
    const { id: sectionID, title: sectionTitle, description, open: sectionCollapse } = section;
    ...
    const body = (
      <div className='min-w-full p-4 pb-0' key={`${sfrType}AuditSection`}>
        ...
        {isSection && sfrType !== "additional" && (
          <div className='pt-4'>
            <FormTextField value={description} label={"Description"} tag={type} handleTextUpdate={handleTextUpdate} />
          </div>
        )}
      </div>
    );
  ...
  // Return Method
  return (
    <div key={`${sfrType}AuditableEvents`} className='w-full'>
      {!isAudit ? (
          ...
          body={
            <div className='min-w-full'>
              ...
              <div className='min-w-full m-0 p-0 px-4 mb-[-2px]'>
                <AuditEventTable baseSfrUUID={uuid} sfrType={sfrType} collapse={eventsTableOpen} handleCollapse={handleAuditEventsTableCollapse} />
              </div>
            </div>
          }
          collapse={open}
      ...
      <ResetDataConfirmation
        ...
        text={`Are you sure you want to reset all ${dialogTitle} SFR Auditable Events data to its initial state?`}
        open={openResetData}
