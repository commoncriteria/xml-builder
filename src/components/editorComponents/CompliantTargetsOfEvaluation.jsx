...
function CompliantTargetsOfEvaluation(props) {
    ...
    // Constants
    const { getCardTemplate, handleSnackBarError, handleSnackBarSuccess } = SecurityComponents
    ...
    const {
        ...
        additionalText,
        open
    } = useSelector((state) => state.compliantTargetsOfEvaluation);
    ...
    const handleUpdateTableRowNotes = (event) => {
        ...
        try {
            dispatch(UPDATE_ROW_DATA_NOTES_BY_INDEX({
                index: rowIndex,
                value: value
            }))
        } catch (e) {
    ...
    const deleteTOEOverview = () => {
        try {
            ...
            if (accordionUUID && uuid) {
                ...
                dispatch(DELETE_ACCORDION_FORM_ITEM({
    ...
    return (
        <div
            ...
            key={"compliantTargetsOfEvaluationDiv"}
        >
            <Card className="h-full w-full rounded-lg border-2 border-gray-300">
                ...
                >
                    <div className="flex">
                        <label className="mr-2 resize-none font-bold text-[14px] text-secondary">
                            {props.section}
                        </label>
                        <span/>
                        <Tooltip
                        ...
                        >
                            <Tooltip
                                ...
                                id={"deleteTOEOverviewSectionButton"}
                            >
                        ...
                        >
                            <Tooltip
                                title={`${open ? "Collapse " : "Expand "} TOE Overview`}
                                id={(open ? "collapse" : "expand") + "TOEOverviewTooltip"}
                            >
                                { open ?
            ...
            <DeleteConfirmation
                ...
                handleSubmit={deleteTOEOverview}
            />
        </div>
    )
}
