...
function Terms(props) {
  ...
  Terms.propTypes = {
    ...
    showAddButton: PropTypes.bool, // used for CC terms, to disable addition of terms
  };
  ...
  return (
    <div className='min-w-full mb-2' key={props.uuid + "Div"}>
      <Card className='h-full w-full rounded-lg border-2 border-gray-300'>
        <CardBody className='mb-0 rounded-b-none' key={props.uuid + "CardBody"}>
          <div className='flex'>
            ...
            <span />
            <Tooltip title={props.titleTooltip ? props.titleTooltip : ""} id={props.uuid + props.titleTooltip} arrow disableHoverListener={!props.titleTooltip}>
              <textarea
                className='w-full resize-none font-bold text-[14px] mb-0 h-[30px] p-0 text-secondary'
                onChange={updateTermsListTitle}
                value={props.title}>
                {props.title}
              </textarea>
            </Tooltip>
      ...
      <DeleteConfirmation title={props.title} open={openDeleteDialog} handleOpen={() => setDeleteDialog(!openDeleteDialog)} handleSubmit={deleteTermsList} />
    </div>
  );
}
