...
import { getCardTemplate, handleSnackBarSuccess, handleSnackBarError } from "../../utils/securityComponents.jsx";
...
function ConformanceClaims() {
  ...
  const getConformanceSection = () => {
    ...
    const body = (
      <div className='min-w-full p-4 pr-1 pb-0'>
        ...
        <div className='flex mt-4'>{getDropdownByType(cc_errata, "Select cc-errata", "cc_errata", ccErrataOptions, false)}</div>
      </div>
    );
    ...
    return getCardTemplate(header, body, tooltip, collapseConformanceSection, () => handleSectionCollapse("conformation"));
  };
  /**
  ...
  const getAdditionalInformationSection = () => {
    ...
    const body = (
      <div className='w-full p-4 pb-0 mb-[-8px]'>
        <TipTapEditor text={additionalInformation} contentType={"term"} handleTextUpdate={handleAdditionalInformationTextUpdate} />
      </div>
    );
    ...
    return getCardTemplate(header, body, tooltip, collapseAdditionalInfoSection, () => handleSectionCollapse("additionalInfo"));
  };
  /**
    ...
    return (
      <div className='w-full'>
        <div className='relative'>
          <EditableTable
            title={
              <Tooltip arrow id={isClaimsTable ? (isPpClaim ? "ppClaimTooltip" : "packageClaimTooltip") : "evaluationMethodsTooltip"}>
  ...
  const getPpClaimSectionTable = useMemo(() => {
    ...
    const columnData = [
      ...
      },
      {
        ...
        flex: 0.75,
        headerTooltip:
          "This section lists the PP that the current document is conformant to, and the PPs and PP-Modules and that the document can be in a PP-Configuration with.",
      },
  ...
  }, [additionalInformation, collapseAdditionalInfoSection]);
