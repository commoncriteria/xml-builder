  ...
  const ProgressStatusIcon = (key, value) => {
    return (
      <Box display='flex' alignItems='center'>
        {value ? <CheckCircleIcon htmlColor={primary} sx={icons.small} /> : <RadioButtonUncheckedIcon htmlColor={primary} sx={icons.small} />}
    ...
    return (
      <Box>
        {Object.entries(steps)
          .slice(start, end)
          .map(([key, value], index) => (
            <Box key={`col-${index}`}>{ProgressStatusIcon(key, value)}</Box>
          ))}
      </Box>
    );
  };
    ...
    <div>
      {isLoading ? (
        <div className='mt-2 w-full'>
          <Box sx={{ width: "100%" }}>
            <label>* Warning: If the dialog is closed prematurely, all data will reset to its default state</label>
            <Box>
