...
export const accordionPaneSlice = createSlice({
  ...
  reducers: {
    CREATE_ACCORDION: (state, action) => {
      ...
      const { title, selected_section, custom, isAppendix, xmlTagMeta } = action.payload;

      let uuid = uuidv4();
      let index = Object.values(state.sections).findIndex((value) => value.title === selected_section);
      ...
      if (index !== -1) {
        ...
        keyValues.splice(index + 1, 0, [
          uuid,
          { title: isAppendix ? `Appendix - ${title}` : title, open: false, formItems: [], custom, selected_section, isAppendix, xmlTagMeta },
        ]);
        state.sections = Object.fromEntries(keyValues);
      } else {
        state.sections[uuid] = {
          title: isAppendix ? `Appendix - ${title}` : title,
          open: false,
          formItems: [],
          custom,
          selected_section,
    ...
    CREATE_ACCORDION_FORM_ITEM: (state, action) => {
      const { accordionUUID, uuid, contentType } = action.payload;
      ...
      if (state.sections.hasOwnProperty(accordionUUID)) {
        ...
        if (contentType === "sfrBasePPs") {
          addSfrBasePP(accordion.formItems, {
            uuid,
            contentType,
          });
        } else {
    ...
    CREATE_ACCORDION_SFR_FORM_ITEM: (state, action) => {
      ...
      if (accordionUUID && editorUUID && sfrUUID && state.sections.hasOwnProperty(accordionUUID) && state.sections[accordionUUID].hasOwnProperty("formItems")) {
    ...
    CREATE_ACCORDION_SFR_MODULE_FORM_ITEM: (state, action) => {
      updateInnerSecurityFormItems(state, action);
    },
    CREATE_ACCORDION_SAR_MODULE_FORM_ITEM: (state, action) => {
      updateInnerSecurityFormItems(state, action);
    },
    CREATE_ACCORDION_SAR_FORM_ITEM: (state, action) => {
      ...
      if (accordionUUID && editorUUID && sarUUID && state.sections.hasOwnProperty(accordionUUID) && state.sections[accordionUUID].hasOwnProperty("formItems")) {
        let formItems = state.sections[accordionUUID].formItems;
        if (formItems && formItems.length > 0) {
          formItems.map((item) => {
            let uuid = item.uuid;
    ...
    GET_ACCORDION_SFR_FORM_ITEMS: (state, action) => {
      ...
      let editorUUID = action.payload.editorUUID;
      action.payload.sfrs = [];
      if (accordionUUID && editorUUID && state.sections.hasOwnProperty(accordionUUID) && state.sections[accordionUUID].hasOwnProperty("formItems")) {
        let formItems = state.sections[accordionUUID].formItems;
        if (formItems && formItems.length > 0) {
          formItems.map((item) => {
            let uuid = item.uuid;
...
// Internal Methods
/**
 * Adds the sfr base pp above the editors section if it exists
 * @param formItems the form items
 * @param newObject the new object
 * @returns {*}
 */
function addSfrBasePP(formItems, newObject) {
  // Find the index of the first object above the "editor" section
  const targetIndex = formItems.findIndex((item) => item.contentType === "editor");

  // If an object with editor exists, insert the new object above it
  if (targetIndex !== -1) {
    formItems.splice(targetIndex, 0, newObject);
  } else {
    // If no such object exists, add the new object at the end of the array
    formItems.push(newObject);
  }

  return formItems;
}
/**
 * Updates the inner security form items
 * @param state the state
 * @param action the action
 */
const updateInnerSecurityFormItems = (state, action) => {
  const { accordionUUID, formUUID, innerFormUUID, newUUID, contentType } = action.payload;
  const isAccordionValid = accordionUUID && state.sections.hasOwnProperty(accordionUUID) && state.sections[accordionUUID].hasOwnProperty("formItems");
  const isValid = isAccordionValid && formUUID && innerFormUUID && newUUID && contentType;

  if (isValid) {
    let formItems = deepCopy(state.sections[accordionUUID].formItems);
    const formItemsIndex = formItems.findIndex((item) => item.uuid === formUUID);

    if (formItemsIndex !== -1 && formItems[formItemsIndex].hasOwnProperty("formItems")) {
      let innerFormItems = formItems[formItemsIndex].formItems;
      const innerFormItemsIndex = innerFormItems.findIndex((item) => item.uuid === innerFormUUID);

      if (innerFormItemsIndex !== -1) {
        innerFormItems = formItems[formItemsIndex].formItems[innerFormItemsIndex];

        if (!innerFormItems.hasOwnProperty("formItems")) {
          innerFormItems.formItems = [];
        }

        innerFormItems.formItems.push({
          uuid: newUUID,
          contentType: contentType,
        });

        // Update state form items
        state.sections[accordionUUID].formItems = formItems;
      }
    }
  }
};
